...
# ── Robust path setup — works on Streamlit Cloud and locally ──────────────────
_HERE = os.path.dirname(os.path.abspath(__file__))
# Streamlit Cloud mounts repo at /mount/src/<reponame>/
# Try all likely locations for the views folder
_CANDIDATES = [
    _HERE,
    os.path.join(_HERE, "ici_dashboard"),
    os.getcwd(),
    os.path.join(os.getcwd(), "ici_dashboard"),
    "/mount/src/intgli",
    "/mount/src/intgli/ici_dashboard",
]
# Find the directory that actually contains views/
_ROOT = next(
    (p for p in _CANDIDATES if os.path.isdir(os.path.join(p, "views"))),
    _HERE  # fallback
)
...
def _import_module(module_name):
    """Import a module by name, searching _ROOT and its subdirs."""
    import importlib.util
    # Try direct import first (works if sys.path is set correctly)
    try:
        return __import__(module_name, fromlist=["render"])
    except ModuleNotFoundError:
        pass
    # Try loading by file path
    parts = module_name.split(".")
    if len(parts) == 2:
        path = os.path.join(_ROOT, parts[0], f"{parts[1]}.py")
    else:
        path = os.path.join(_ROOT, f"{module_name}.py")
    if os.path.exists(path):
        spec = importlib.util.spec_from_file_location(module_name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod
    st.error(f"Cannot find module '{module_name}'. ROOT={_ROOT}")
    st.stop()


def _get_engine_class():
    return _import_module("data_engine").DataEngine
...
""", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f'<div style="background:white;border:2px dashed {TEAL}44;border-radius:16px;padding:24px;text-align:center;margin-bottom:8px"><div style="font-size:10px;text-transform:uppercase;letter-spacing:.2em;color:{TEAL};font-weight:700;margin-bottom:8px">REQUIRED</div><div style="font-size:16px;font-weight:600;color:#0F172A;margin-bottom:4px">ATU Workbook</div><div style="font-size:12px;color:#64748B">Q1 + Q2 · .xlsx</div></div>', unsafe_allow_html=True)
        atu_file = st.file_uploader("ATU", type=["xlsx"], key="atu_hero", label_visibility="collapsed")
    with c2:
        st.markdown(f'<div style="background:white;border:2px dashed {TEAL}44;border-radius:16px;padding:24px;text-align:center;margin-bottom:8px"><div style="font-size:10px;text-transform:uppercase;letter-spacing:.2em;color:{TEAL};font-weight:700;margin-bottom:8px">REQUIRED</div><div style="font-size:16px;font-weight:600;color:#0F172A;margin-bottom:4px">PET Workbook</div><div style="font-size:12px;color:#64748B">Q4 + Q1 + Q2 · .xlsx</div></div>', unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if eng.is_loaded:
            ...
            st.markdown("---")
            s = eng.stats()
            st.markdown(f'<div style="font-size:11px;color:#64748B;line-height:1.8"><div>ATU HCPs: <b>{s["atu_n"]}</b></div><div>PET interactions: <b>{s["pet_n"]}</b></div><div>Overlapping: <b style="color:{TEAL}">{s["overlap_n"]}</b></div></div>', unsafe_allow_html=True)
        else:
            st.info("Upload ATU + PET files on the main screen to begin.")

    # No data → upload screen
    if not eng.is_loaded:
        st.markdown(f'<div class="topbar"><div class="tb-logo">🏥</div><div><div class="tb-sub">Interaction Conversion Index · Intelligence</div><div class="tb-title">Healthcare Professional Conversion Atlas</div></div></div>', unsafe_allow_html=True)
        upload_screen()
        return
    ...
    # Route to view — using _import_module for robust loading
    if cur == "approach":
        _import_module("views.approach").render(eng)
    elif cur == "overview":
        _import_module("views.overview").render(eng, hcps)
    elif cur.startswith("cluster_"):
        _import_module("views.cluster_detail").render(int(cur.split("_")[1]), hcps, eng)
    elif cur == "integrated":
        _import_module("views.integrated").render(eng, hcps)
    elif cur == "crosstabs":
        _import_module("views.crosstabs").render(eng)
    elif cur == "qualitative":
        _import_module("views.qualitative").render(eng)
    elif cur == "envelope":
        _import_module("views.envelope").render(eng, hcps)
    else:
        _import_module("views.overview").render(eng, hcps)
